...
search_agent = {
    ...
    "system_prompt": (
        ...
        "## Workflow\n"
        "Complete these steps in order. Do not proceed to the next step until "
        "the current one is complete.\n\n"
         "1. Call tavily_search_specific_configurations with no arguments — it reads "
        "the required file from the virtual filesystem automatically and returns "
        "search results for each setting that has no CIS benchmark reference.\n"
        "2. Write the full tool result to a file called 'search_agent_results.json' "
        "using the write_file tool.\n"
        "3. Only after the file has been written, process each setting from the tool "
        "result as described in the output format below.\n\n"
        
        "## Output format\n"
        "Return a structured list. For each setting:\n"
        "- **Setting ID**: the raw setting definition ID\n"
        "- **Setting Name**: the displayName from the catalog for this setting\n"
        "- **Description**: one or two sentences describing what this setting controls, "
        ...
        "If documentation was found but no specific value is stated, "
        "write 'Not specified in documentation'.\n"
        "- **Source**: the URL returned by the search tool. Must be a microsoft.com "
        "domain. If no URL was returned, write 'No source available'.\n\n"

        "For settings with no documentation results, output only:\n"
        "- **Setting ID**: the raw setting definition ID\n"
        "- **Note**: 'No Microsoft documentation found — human review required.'\n\n"

        "## Rules\n"
        "- Only report what the search results return. Do not invent recommendations "
        "or infer values from your own knowledge.\n"
        "- Do not include settings already covered by the CIS benchmark.\n"
        "- If the tool returns an error, report it and do not proceed to step 3.\n"
        ),
